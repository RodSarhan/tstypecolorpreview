...
import {
    buildDeclarationString,
    getPrettifyType,
    formatDeclarationString,
} from "./helpers";
import { getProject } from "./project-cache";

export async function prettifyType(
    fileName: string,
    content: string,
    offset: number
    ...
    const prettifiedTypeAlias = sourceFile
        .getTypeAliasOrThrow(`PrettifiedType_${prettifyId}`)
        .getTypeNodeOrThrow();
    ...
    if (prettifiedType.isUnion()) {
        ...
        const unionTypeNames = unionTypes.map((unionType) =>
            unionType.getText()
        );
    ...
    const declarationString = buildDeclarationString(
        parentNodeKind,
        nodeText,
        prettifiedTypeString
    );
    const typeString = formatDeclarationString(
        declarationString,
        typeIndentation
    );
